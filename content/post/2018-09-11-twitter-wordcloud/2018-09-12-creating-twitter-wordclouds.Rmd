---
title: Creating Twitter Wordclouds
author: Mikey Harper
description: Guide on how to make wordclouds using the rtweet package in R.
date: '2018-12-12'
...
header_image: "https://i.imgur.com/PLz9aN2.png"
---

<meta property="og:image" content="https://i.imgur.com/PLz9aN2.png"/>
...
Having cleaned the data, we can format the table. The function 'TermDocumentMatrix' is used to construct a frequency table of the wprds from the text string above. This table is sorted by frequency to make it easier to inspect. A quick summary of the most common words is shown in Table \@ref(tab:keywords).
...
We can play around with this basic setup, and I would recommend checking out the [package documentation](https://cran.r-project.org/web/packages/wordcloud2/vignettes/wordcloud.html) to see some of the things that can be done. For example, we can provide our own image as a mask to customise the shape of the wordcloud. The output is shown in Figure \@ref(fig:wordCloud2).

```{r wordCloud2, fig.cap = "Wordcloud with our Twitter outline"}
wordcloud_twitter <- wordcloud2(data = textCorpus, figPath = "twitter.png")
wordcloud_twitter
```
