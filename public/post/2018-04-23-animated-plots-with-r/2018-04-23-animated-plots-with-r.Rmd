...
Data scientists are blessed with a range of software tools which they can use to visualise results, and allow users to interact with results. Although many people will commonly use static graphs like **ggplot2** [@R-ggplot2], animated plots can be an effective way to communicate results, and there are few better examples than the use of them in [the now famous TED talk by Hans Rosling](https://www.youtube.com/watch?v=hVimVzgtD6w).
...
The [ggplot facet](http://ggplot2.tidyverse.org/reference/facet_grid.html) is a commonly used tool for displaying similar datasets. However, an animation can also be used to visualise the results, as shown in Figure \@ref(fig:animatedMap)
...
saveGIF({
  for (i in unique(REPD$`Technology Type`)){
  
  df <- REPD_tech %>%
    filter(`Technology Type` == i)
  ...
  p <- 
    ...
    # Highlight Reference Year
    geom_point(data = df, 
               fill = "#176a90", size = 5, shape = 21, alpha = 0.8) +
...
}, movie.name = "renewableTech.gif",
  interval = 2,  ani.width = 750,  ani.height = 1000
)
```

```{r animatedMap, fig.cap="An Animated Map", out.width="60%", fig.align = "center"}
knitr::include_graphics("https://mikeyharper.uk/post/2018-04-23-animated-plots-with-r/renewableTech.gif")
```

If want to use **ggplot**, and only want to animate a single graph, then **gganminate** [@R-gganimate] may be worth exploring, as it provides a simplifed way of constructing the Gif.
...
2. A barplot shows the total installed capacity of wind turbines within the specified year
...
saveGIF({
  ...
  p <- 
    ...
    # Plot all points before reference year
...
An example of this package is beyound the scope of this article, but you may want to check out the examples on the [GitHub page](https://github.com/thomasp85/tweenr), or alternatively [this blog](http://blog.revolutionanalytics.com/2017/05/tweenr.html) provides some cool examples.
